...
// Icon map: CMS icon name → SVG element
...
const DEFAULT_BENEFITS = [
  { icon: 'sparkles', text: 'Book Instantly' },
...
export const HeroSection = () => {
  ...
  useEffect(() => {
    fetchSiteContentClient().then((c) => {
      ...
      // Hero features (benefit pills)
      const heroFeatures = (c.contentItems?.['hero_features'] || [])
        .filter((item) => item.isActive)
        .sort((a, b) => a.order - b.order);
      if (heroFeatures.length > 0) {
        setBenefits(heroFeatures.map((item) => ({
      ...
      <div className="hero-content">
        ...
        >
          {benefits.map((benefit, index) => (
            ...
            >
              <span className="hero-benefit-emoji">{ICON_MAP[benefit.icon] || ICON_MAP['sparkles']}</span>
